...
| 선행② | KOTRA 해외시장뉴스 | 실시간 정책·규제 동향 감지 | 90일 884건 중 공급망 매칭 60건 |
...
| 모듈 | 역할 |
|---|---|
| 뉴스 신호 (`news_signal.py`) | **실시간 정책·규제 동향 감지** |
| 구조 진단 (`scripts/step3_backtest.py`) | **과거 사태 소급 검증** — 관세청 HHI 기반 |

뉴스 신호는 지금 일어나고 있는 정책·규제 움직임을 잡는 역할만 맡습니다.
과거 사태 소급 검증은 관세청 HHI 기반 구조 진단이 담당합니다.

- 뉴스 신호 실시간 테스트: **요소수 기준 RED 정상 감지 (KOTRA 3건 + Google 35건)**
  — 모듈 검증 결과로 전달받은 값이며, 원문은 `data/news_signal_test.json`에 있습니다.
- 구조 진단 소급 검증: 관세청 원자료 42개월 구간에서 급등이 발생한 13개 품목 중
  **5개를 사전 탐지**(선행 개월 중앙값 2.0개월). 산출물은
  `data/processed/backtest_leadtime.csv`입니다.
